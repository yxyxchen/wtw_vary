...
modelRepitation = function(modelName, summaryData, trialData,  nComb, blockNum){
  # determine repFun
  ...
  for(sIdx in 1 : nSub){
    ...
    for(cbIdx in 1 : nComb){
      ...
      tempt = repFun(paras, condition, scheduledWait, scheduledReward, blockNum)
...
QL1 = function(paras, condition, scheduledWait, scheduledReward, blockNum){
  # parse paras
  ...
  outputs = list( 
    ...
    "Vitis" = Vitis, "condition" = condition, "blockNum" = blockNum
  )
  return(outputs)
}
...
RL1 = function(paras, condition, scheduledWait, scheduledReward, blockNum){
  # parse para
  ...
  outputs = list( 
    ...
    "sellTime" = sellTime, "scheduledWait" = scheduledWait,
    "Qwaits" = Qwaits, "targets" = targets, "deltas" = deltas,
    "Vitis" = Vitis, "reRates" = reRates, "condition" = condition, "blockNum" = blockNum
  )
  return(outputs)
}
