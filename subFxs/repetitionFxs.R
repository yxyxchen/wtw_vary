...
modelRepitation = function(modelName, summaryData, expTrialData,  nComb){
  ...
  for(sIdx in 1 : nSub){
    ...
    excluedTrialsHP = which(thisExpTrialData$trialStartTime > (blockSecs - tMaxs[1]) &
                              thisExpTrialData$condition == conditions[1])
    excluedTrialsLP = which(thisExpTrialData$trialStartTime > (blockSecs - tMaxs[2]) &
                              thisExpTrialData$condition == conditions[2])
    ...
    blockNum = thisExpTrialData$blockNum
    ...
    for(cbIdx in 1 : nComb){
      ...
      tempt = repModelFun(paraSample, cond, blockNum, scheduledWait, scheduledReward)
...
reduce_gamma = function(paras, cond, blockNum, scheduledWait, scheduledReward){
  # parse para
  ...
  outputs = list( 
    ...
    "cond" = cond, "blockNum" = "blockNum"
  )
  return(outputs)
}


Rlearn = function(paras, cond, blockNum, scheduledWait, scheduledReward){
  # parse para
  phi = paras[1]; phiP = paras[2]; tau = paras[3]; zeroPoint = paras[4]
  ...
  outputs = list( 
    ...
    "Vitis" = Vitis, "reRates" = reRates, "cond" = cond, "blockNum" = blockNum
  )
  return(outputs)
}


RlearnL = function(paras, cond, blockNum, scheduledWait, scheduledReward){
  # parse para
  phi = paras[1]; phiP = paras[2]; tau = paras[3]; zeroPoint = paras[4]
...
#### PRbs
PRbsNC = function(paras, cond, blockNum, scheduledWait, scheduledReward){
  # parse para
...
PRbs = function(paras, cond, blockNum, scheduledWait, scheduledReward){
  # parse para
  phi = paras[1]; phiP = paras[2]; tau = paras[3]; gamma = paras[4]; zeroPoint = paras[5]
