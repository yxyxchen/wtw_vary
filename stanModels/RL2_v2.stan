...
transformed parameters{
  ...
  real beta_pos = phi_pos * beta_ratio; // convert beta_ratio to beta
  real beta_neg = phi_neg * beta_ratio; // convert beta_ratio to beta
  ...
  real currentBeta; // the learning rate for reward rates for this trial
  ...
  for(tIdx in 1 : (N -1)){
    ...
    if(R > 0){
      currentPhi = phi_pos;
      currentBeta = beta_pos;
    }else{
      currentPhi = phi_neg;
      currentBeta = beta_neg;
    }
    ...
    reRate = reRate + currentBeta * itiDelta;
